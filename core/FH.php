...
class FH {
  ...
  public static function inputBlock($type, $label, $name, $value='', $inputAttrs=[], $divAttrs=[],$errors=[]){
    ...
    if(array_key_exists('id',$inputAttrs)){
      $id=$inputAttrs['id'];
    }else{
      $id = str_replace('[]','',$name);
    }
  ...
  public static function submitTag($buttonText, $inputAttrs=[]){
    $inputString = self::stringifyAttrs($inputAttrs);
    $html = '<input type="submit" name="'.$buttonText.'" value="'.$buttonText.'"'.$inputString.' />';
    return $html;
  }
  ...
  public static function checkboxBlock($label,$name,$checked=false,$inputAttrs=[],$divAttrs=[],$errors=[]){
    ...
    $id = str_replace('[]','',$name);
    $html = '<div'.$divString.'>';
    $html .= '<label for="'.$id.'" class="control-label">'.$label.' <input type="checkbox" id="'.$id.'" name="'.$name.'" '.$checkString.$inputString.'>';
    $html .= '<span class="invalid-feedback">'.self::errorMsg($errors,$name).'</span>';
    $html .= '</label>';
    $html .= '</div>';
    return $html;
  ...
  public static function selectBlock($label,$name,$value,$options,$inputAttrs=[],$divAttrs=[],$errors=[]){
    ...
    $html .= '<select id="'.$id.'" name="'.$name.'" value="'.$value.'"'.$inputString.'>'.self::optionsForSelect($options,$value).'</select>';
    $html .= '<span class="invalid-feedback">'.self::errorMsg($errors,$name).'</span>';
    $html .= '</div>';
    return $html;
  }
  ...
  public static function sanitize($dirty) {
   //var_dump( htmlentities($dirty, ENT_IGNORE, 'UTF-8'));
   if(!is_array($dirty)){
    if($dirty=='on')
      $dirty=1;
  ...
  public static function displayErrors($errors) {
    ...
    $html = '<div class="form-errors m-0"><ul class="bg-light'.$hasErrors.'">';
   ...
   */
  public static function optionsForSelect($options,$id, $defaultMsg="Seleccionar"){
    $html = "";
    $html .= '<option value="">'.$defaultMsg.'</option>';
    foreach((array)$options as $value => $display){
      if( html_entity_decode($value)==html_entity_decode($id)){
