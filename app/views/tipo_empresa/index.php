<?php $this->setSiteTitle('Tipos de empresas')?>
...
<script>
   ...
   function guardar(){
      if($("#frmTipoEmpresa").valid()){
         var form = jQuery('#frmTipoEmpresa').serialize();
         jQuery.ajax({
            url : '<?=PROOT?>tipoEmpresa/nuevo',
            method: "POST",
            data : form,
            success: function(resp){
               if(resp.success){
                  ...
                  setTimeout(function () {
                         window.location.href = '<?=PROOT?>tipoEmpresa'; //will redirect to your blog page (an ex: blog.html)
                      }, 1500);
               }else{
   ...
   function eliminar(id){
      ...
      function(isConfirm) {
         if (isConfirm) {
            jQuery.ajax({
               url : '<?=PROOT?>tipoEmpresa/eliminar',
               method: "POST",
               data : {id:id},
               success: function(resp){
                  if(resp.success){
