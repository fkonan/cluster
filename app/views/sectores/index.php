<?php $this->setSiteTitle('Listado de Sectores')?>
      ...
      <div class="table-responsive">
         <table class="table table-striped table-condensed table-bordered table-hover">
            <thead class="text-center">
               <th class="col-auto bg-success">Sector</th>
...
<script>
   ...
   function guardar(){
      if($("#frmSectores").valid()){
         var form = jQuery('#frmSectores').serialize();
         jQuery.ajax({
            url : '<?=PROOT?>sectores/nuevo',
            method: "POST",
            data : form,
            success: function(resp){
               if(resp.success){
                  ...
                  setTimeout(function () {
                         window.location.href = '<?=PROOT?>sectores'; //will redirect to your blog page (an ex: blog.html)
                      }, 1500);
               }else{
