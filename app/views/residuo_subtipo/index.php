<?php $this->setSiteTitle('Listado de subtipos de residuos')?>
...
<div class="card border-info">
   <div class="card-header text-center bg-success text-white">
      Listado de subtipos de residuos
   </div>
      ...
      <div class="table-responsive">
         <table class="table table-striped table-condensed table-bordered table-hover">
            <thead class="text-center">
               <th class="col-auto bg-success">Tipo de residuo</th>
               <th class="col-auto bg-success">Subtipo de residuo</th>
...
<script>
   function nuevo(){  
      jQuery.ajax({
         url : '<?=PROOT?>residuoSubTipo/nuevo',
         method : "GET",
   ...
   function editar(id){ 
      jQuery.ajax({
         url : '<?=PROOT?>residuoSubTipo/editar',
         data:{id:id},
         method : "GET",
         success : function(resp){
            jQuery('#bodyModal').html(resp);
   ...
   function guardar(){
      if($("#frmResiduoSubTipo").valid()){
         var form = jQuery('#frmResiduoSubTipo').serialize();
         jQuery.ajax({
            url : '<?=PROOT?>residuoSubTipo/nuevo',
            method: "POST",
            data : form,
            success: function(resp){
               if(resp.success){
                  ...
                  setTimeout(function () {
                         window.location.href = '<?=PROOT?>residuoSubTipo';
                      }, 1500);
               }else{
   ...
   function eliminar(id){
      ...
      function(isConfirm) {
         if (isConfirm) {
            jQuery.ajax({
               url : '<?=PROOT?>residuoSubTipo/eliminar',
               method: "POST",
               data : {id:id},
               success: function(resp){
                  if(resp.success){
   ...
   jQuery('#frmModal').on('show.bs.modal', function(){
      $("#frmResiduoSubTipo").validate({
         ...
         messages: {
            tipo_id: {
               required:"La tipo es requerida."
            },
            subtipo: {
               required:"El subtipo de residuo es requerido."
            }
         }
      });
   });
