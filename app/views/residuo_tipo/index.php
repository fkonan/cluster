<?php $this->setSiteTitle('Listado de tipos de residuos')?>
...
<script>
   ...
   function guardar(){
      if($("#frmResiduoTipo").valid()){
         var form = jQuery('#frmResiduoTipo').serialize();
         jQuery.ajax({
            url : '<?=PROOT?>residuoTipo/nuevo',
            method: "POST",
            data : form,
            success: function(resp){
               if(resp.success){
                  ...
                  setTimeout(function () {
                         window.location.href = '<?=PROOT?>residuoTipo';
                      }, 1500);
               }else{
   ...
   function eliminar(id){
      ...
      function(isConfirm) {
         if (isConfirm) {
            jQuery.ajax({
               url : '<?=PROOT?>residuoTipo/eliminar',
               method: "POST",
               data : {id:id},
               success: function(resp){
                  if(resp.success){
   ...
   jQuery('#frmModal').on('show.bs.modal', function(){
      $("#frmResiduoTipo").validate({
         ...
         messages: {
            ...
            tipo: {
               required:"El tipo de residuo es requerido."
            }
         }
      });
   });
