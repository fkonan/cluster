<?php $this->setSiteTitle('Listado de tipos de materiales')?>
      ...
      <div class="table-responsive">
         <table class="table table-striped table-condensed table-bordered table-hover" >
            <thead class="text-center">
               <th class="col-auto bg-success">Categoria del material</th>
...
<script>
   ...
   function guardar(){
      if($("#frmMaterialTipo").valid()){
         var form = jQuery('#frmMaterialTipo').serialize();
         jQuery.ajax({
            url : '<?=PROOT?>materialTipo/nuevo',
            method: "POST",
            data : form,
            success: function(resp){
               if(resp.success){
                  ...
                  setTimeout(function () {
                         window.location.href = '<?=PROOT?>materialTipo';
                      }, 1500);
               }else{
   ...
   function eliminar(id){
      ...
      function(isConfirm) {
         if (isConfirm) {
            jQuery.ajax({
               url : '<?=PROOT?>materialTipo/eliminar',
               method: "POST",
               data : {id:id},
               success: function(resp){
                  if(resp.success){
   ...
   jQuery('#frmModal').on('show.bs.modal', function(){
      $("#frmMaterialTipo").validate({
         ...
         messages: {
            ...
            tipo: {
               required:"El tipo de material es requerido."
            }
         }
      });
   });
