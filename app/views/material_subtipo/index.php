<?php $this->setSiteTitle('Listado de subtipos de materiales')?>
...
<div class="card border-info">
   <div class="card-header text-center bg-success text-white">
      Listado de subtipos de materiales
   </div>
      ...
      <div class="table-responsive">
         <table class="table table-striped table-condensed table-bordered table-hover">
            <thead class="text-center">
               <th class="col-auto bg-success">Tipo de material</th>
               <th class="col-auto bg-success">Subtipo de material</th>
...
<script>
   function nuevo(){  
      jQuery.ajax({
         url : '<?=PROOT?>materialSubTipo/nuevo',
         method : "GET",
   ...
   function editar(id){ 
      jQuery.ajax({
         url : '<?=PROOT?>materialSubTipo/editar',
         data:{id:id},
         method : "GET",
         success : function(resp){
            jQuery('#bodyModal').html(resp);
   ...
   function guardar(){
      if($("#frmMaterialSubTipo").valid()){
         var form = jQuery('#frmMaterialSubTipo').serialize();
         jQuery.ajax({
            url : '<?=PROOT?>materialSubTipo/nuevo',
            method: "POST",
            data : form,
            success: function(resp){
               if(resp.success){
                  ...
                  setTimeout(function () {
                         window.location.href = '<?=PROOT?>materialSubTipo';
                      }, 1500);
               }else{
   ...
   function eliminar(id){
      ...
      function(isConfirm) {
         if (isConfirm) {
            jQuery.ajax({
               url : '<?=PROOT?>materialSubTipo/eliminar',
               method: "POST",
               data : {id:id},
               success: function(resp){
                  if(resp.success){
   ...
   jQuery('#frmModal').on('show.bs.modal', function(){
      $("#frmMaterialSubTipo").validate({
         ...
         messages: {
            tipo_id: {
               required:"El tipo de material es requerida."
            },
            subtipo: {
               required:"El subtipo de material es requerido."
            }
         }
      });
   });
