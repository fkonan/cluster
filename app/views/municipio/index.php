<?php $this->setSiteTitle('Listado de municipios')?>
...
<div class="card border-info">
   <div class="card-header text-center bg-success text-white">
      <strong>Listado de municipios</strong>
   </div>
   <div class="card-body pt-2">
      <a href="#" onClick="nuevo();" class="btn btn-info btn-md float-right mb-2">Nuevo registro</a>
      <div class="table-responsive">
         <table class="table table-striped table-condensed table-bordered table-hover"  data-filtering="true">
            <thead class="text-center">
               <th class="col-auto bg-success">Municipio</th>
...
<script>
   ...
   function guardar(){
      if($("#frmMunicipio").valid()){
         var form = jQuery('#frmMunicipio').serialize();
         jQuery.ajax({
            url : '<?=PROOT?>municipio/nuevo',
            method: "POST",
            data : form,
            success: function(resp){
               if(resp.success){
                  ...
                  setTimeout(function () {
                         window.location.href = '<?=PROOT?>municipio';
                      }, 1500);
               }else{
